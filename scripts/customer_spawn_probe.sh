...
BID_FLAG="${BID_FLAG:-0}"
ACTUAL_FARE="${ACTUAL_FARE:-5}"
VEHICLE_CATEGORY_ID="${VEHICLE_CATEGORY_ID:-80787aa9-10b4-4af5-81a8-6c79a8acf154}"
...
OUT="$(curl -sS -X POST "$BASE/api/customer/ride/create" \
  ...
  --data-urlencode "bid=$BID_FLAG" \
  --data-urlencode "type=$TYPE" \
  --data-urlencode "vehicle_category_id=$VEHICLE_CATEGORY_ID" \
  --data-urlencode "actual_fare=$ACTUAL_FARE" \
