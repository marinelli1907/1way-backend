...
<div class="container-fluid">
    @include('adminmodule::components.admin.page-header', ['title' => 'Payout Rules', 'subtitle' => 'Who funds what.'])
    ...
    <div class="card oneway-card">
        <div class="card-header bg-transparent border-0"><h5 class="mb-0 fw-semibold">Rules</h5></div>
        <div class="card-body p-0">
            <div class="table-responsive">
                <table class="table table-hover mb-0">
                    <thead class="table-light"><tr><th>Rule</th><th>Partner %</th><th>Platform %</th><th>Status</th><th>Updated</th></tr></thead>
                    <tbody>
                        @forelse(($items ?? collect()) as $item)
                        <tr>
                            <td>{{ $item->name ?? $item['name'] ?? '—' }}</td>
                            <td>{{ $item->partner_pct ?? $item['partner_pct'] ?? '—' }}</td>
                            <td>{{ $item->platform_pct ?? $item['platform_pct'] ?? '—' }}</td>
