...
<div class="main-content">
    <div class="container-fluid">
        ...
        <div class="text-center py-4">
            ...
            <h2 class="fw-bold">Driver Account Created!</h2>
            <p class="text-muted">
            ...
            <div class="col-lg-6">
                <div class="card shadow-sm border border-primary">
                    <div class="card-header bg-primary text-white fw-bold">
                        <i class="bi bi-link-45deg me-1"></i>Driver Invite Link
                    </div>
                    <div class="card-body">
                        @if($inviteUrl)
                            <p class="text-muted small mb-2">Share this link with the driver. It expires in 7 days.</p>
                        ...
                        @else
                            <div class="alert alert-warning mb-0">
                                No active invite link. Click "Regenerate" to create one.
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body">
                        @if($onboarding)
                            ...
                            @php
                            $steps = [
                                ['key' => 'profile_complete', 'label' => 'Profile Complete',   'icon' => 'bi-person-fill'],
                                ['key' => 'docs_uploaded',    'label' => 'Documents Uploaded', 'icon' => 'bi-file-earmark-check'],
                                ['key' => 'approved',         'label' => 'Account Approved',   'icon' => 'bi-shield-check'],
                                ['key' => 'active',           'label' => 'Activated & Live',   'icon' => 'bi-play-circle'],
                            ];
                            @endphp
                            ...
                            <ul class="list-group list-group-flush" id="onboardingList">
                                ...
                                <li class="list-group-item d-flex align-items-center gap-3 px-0">
                                    ...
                                    <span class="ms-auto badge {{ $onboarding->{$step['key']} ? 'bg-success' : 'bg-secondary' }} step-badge" id="badge_{{ $step['key'] }}">
                                        {{ $onboarding->{$step['key']} ? 'Done' : 'Pending' }}
                                    </span>
                                </li>
                                @endforeach
                            </ul>
                            ...
                            <div class="mt-3">
                                ...
                                <textarea class="form-control form-control-sm" id="onboardingNotes"
                                          rows="2" placeholder="Optional notes...">{{ $onboarding->notes }}</textarea>
                                <button class="btn btn-sm btn-outline-primary mt-2" onclick="saveNotes('{{ $driver->id }}')">
                                    Save Notes
                                </button>
                            </div>
                        @else
                            <div class="alert alert-warning mb-0">Onboarding record not found.</div>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="text-center mt-4 d-flex gap-3 justify-content-center flex-wrap">
            ...
            <a href="{{ route('admin.driver.quick-add.create') }}" class="btn btn-primary">
                <i class="bi bi-plus-circle me-1"></i>Add Another Driver
            </a>
            <a href="{{ route('admin.driver.index') }}" class="btn btn-outline-secondary">
                <i class="bi bi-list me-1"></i>Driver List
            </a>
        </div>
...
function regenerateInvite(driverId) {
    ...
    .then(data => {
        ...
        alert('New invite link ready!');
    })
    .catch(() => alert('Error regenerating link.'));
}
