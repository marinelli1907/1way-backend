...
//        dd($pickup_coordinates);
        ...
        // 1Way PATCH: compute missing distance/time + provide safe defaults (API/postman friendly)
        // Keep this BEFORE fare logic so downstream uses normalized values.
        if (!$request->filled("estimated_distance")) {
            $lat1 = (float) $request->input("pickup_latitude");
            $lon1 = (float) $request->input("pickup_longitude");
            $lat2 = (float) $request->input("destination_latitude");
            $lon2 = (float) $request->input("destination_longitude");
            ...
            if (is_numeric($lat1) && is_numeric($lon1) && is_numeric($lat2) && is_numeric($lon2)) {
                ...
                $miles = max(0.0, $earth * $c);
                $request->merge(["estimated_distance" => round($miles, 4)]);
            }
        }

        if (!$request->filled("estimated_time")) {
            $dist = (float) $request->input("estimated_distance", 0);
            $avgMph = (float) (get_cache("avg_speed_mph") ?? 25.0);
            if ($avgMph <= 0) { $avgMph = 25.0; }
            $minutes = ($dist > 0) ? max(1.0, ($dist / $avgMph) * 60.0) : 1.0;
            $request->merge(["estimated_time" => round($minutes, 4)]);
        }

        $request->merge([
            "estimated_fare" => (float) $request->input("estimated_fare", 0),
            "actual_fare" => (float) $request->input("actual_fare", $request->input("estimated_fare", 0)),
            "extra_estimated_fare" => (float) $request->input("extra_estimated_fare", $request->input("estimated_fare", 0)),
            "payment_method" => $request->input("payment_method", "cash"),
            "note" => $request->input("note", ""),
            "entrance" => $request->input("entrance", ""),
            "encoded_polyline" => $request->input("encoded_polyline", ""),
            "return_fee" => (float) $request->input("return_fee", 0),
            "cancellation_fee" => (float) $request->input("cancellation_fee", 0),
            "extra_return_fee" => (float) $request->input("extra_return_fee", 0),
            "extra_cancellation_fee" => (float) $request->input("extra_cancellation_fee", 0),
        ]);
        ...
        if ($request->trip_request_id) {
            ...
            if ($request->filled('customer_coordinates')) {
                $tmp = json_decode($request->input('customer_coordinates'), true);
                if (is_array($tmp) && count($tmp) >= 2 && is_numeric($tmp[0]) && is_numeric($tmp[1])) {
                    $customer_coordinates = [$tmp[0], $tmp[1]];
                }
            }

            $lat = $customer_coordinates[0] ?? $request->input('pickup_latitude');
            $lng = $customer_coordinates[1] ?? $request->input('pickup_longitude');

            // Always keep SRID consistent with zones (4326)
            $customer_point = new Point((float) $lat, (float) $lng, 4326);
            $request->merge([
    ...
    public function rideStatusUpdate($trip_request_id, Request $request): JsonResponse
    {
        ...
        if ($trip->current_status == CANCELLED) {
            return response()->json(responseFormatter(TRIP_STATUS_CANCELLED_403), 200);
        }
        if ($trip->current_status == COMPLETED) {
            return response()->json(responseFormatter(TRIP_STATUS_COMPLETED_403), 200);
        }
        if ($trip->current_status == RETURNING) {
            return response()->json(responseFormatter(TRIP_STATUS_RETURNING_403), 200);
        }
        $attributes = [
            'column' => 'id',
    ...
    public function receivedReturningParcel($trip_request_id): JsonResponse
    {
        ...
        if ($trip->current_status == RETURNED) {
            return response()->json(responseFormatter(TRIP_STATUS_RETURNED_403), 200);
        }
        DB::beginTransaction();
